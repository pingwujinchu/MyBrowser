...
//	                    "jdbc:oracle:thin:@192.168.1.2:1521:dbname", "username", "password");
	...
	public static Proxy getRandomProxy(){
		...
		String userName;
		String password;
		List<List<String>> userList = getUser();
		int index = (int) (Math.random()*(userList.size()-1));
		index =index%(userList.size()-1)+1;
		List userRow = userList.get(index);
		userName = (String) userRow.get(0);
		password = (String) userRow.get(1);
		user.setIp(userName);
